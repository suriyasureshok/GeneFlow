"""
Session Management for GeneFlow.

Manages user sessions, conversation history, and agent state with persistence.
"""
...
class Session:
    """
    User session with conversation history and context preservation.
    
    Represents a single conversation session including message history,
    execution context, and metadata. Supports serialization for persistence.
    ...
    Attributes:
        ...
        conversation_history (List): All messages in conversation
        context_data (Dict): Persistent session context
        metadata (Dict): Custom metadata storage
        active (bool): Session status
    
    Methods:
        add_message: Append message to history
        update_context: Store context data
    ...
    def get_context(self, key: str = None) -> Any:
        ...
        Returns:
            Context value or full context dictionary.
        """
        if key:
...
class SessionManager:
    ...
    Thread-safe implementation with automatic session persistence
    and cleanup of expired sessions.
    
    Attributes:
        storage_path (Path): Directory for session files
        max_session_age (timedelta): Maximum session age before cleanup
        sessions (Dict): Active session cache
    
    Methods:
        create_session: Creates new session
        get_session: Retrieves existing session
        get_or_create_session: Gets or creates session
        delete_session: Deletes session
        cleanup_old_sessions: Removes expired sessions
        get_all_sessions: Lists all sessions
    ...
    def get_session(self, session_id: str) -> Optional[Session]:
        """
        Retrieves an existing session and updates last_accessed.
    ...
    def delete_session(self, session_id: str):
        """
        Marks session as inactive and removes from cache.
    ...
    def cleanup_old_sessions(self):
        """
        Removes sessions older than max_session_age.
        
        Called periodically to prevent memory leaks and manage storage.
        """
        with self._lock:
            now = datetime.now()
            expired_sessions = [
